...
export const anomaly = schema.table("anomaly", {
  ...
  internalAuditItemId: bigint("internal_audit_item_id", { mode: "number" })
    .references(() => internalAuditItem.id),
}, (t) => [{
  ...
  internalAuditItemIdx: index("internal_audit_item_idx").using(
    "btree",
    t.internalAuditItemId,
  ),
