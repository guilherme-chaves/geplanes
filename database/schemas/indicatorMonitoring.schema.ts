...
export const indicatorMonitoring = schema.table("indicator_monitoring", {
  ...
  preventiveActionId: integer("preventive_action_id").references(() =>
    preventiveAction.id
  ),
  notApplicable: boolean("not_applicable"),
}, (t) => [{
  indicatorMonitoringUk: unique("indicator_monitoring_uk").on(
    t.indicatorId,
    t.index,
    t.createdAt,
    t.finishedAt,
  ...
  preventiveActionIdx: index("preventive_actio_idx").using(
    "btree",
    t.preventiveActionId,
  ),
}]);
